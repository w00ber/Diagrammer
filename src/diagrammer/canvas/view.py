...
class DiagramView(QGraphicsView):
    ...
    def __init__(self, scene: DiagramScene, parent: QWidget | None = None):
        ...
        self._drag_ext_conn_orig_wps: dict = {}  # external conn instance_id -> original waypoints
        ...
        # Group resize state
        self._group_resizing: bool = False
        self._group_resize_handle: int = -1  # handle index 0-7
        self._group_resize_gid: str | None = None
        self._group_resize_start_rect: QRectF | None = None
        self._group_resize_start_mouse: QPointF | None = None
        self._group_resize_member_starts: list[tuple] = []  # (item, pos, w, h, verts)
        ...
        self.setBackgroundBrush(QBrush(app_settings.current_background_color()))
    ...
    def drawBackground(self, painter: QPainter, rect) -> None:
        ...
        if self._grid_visible:
            ...
            draw_grid(
                ...
                self._grid_spacing,
                self.current_scale(),
                minor_color=app_settings.current_grid_color(),
                major_color=app_settings.current_grid_major_color(),
            )

    def drawForeground(self, painter: QPainter, rect) -> None:
        """Draw group bounding box and resize handles when a group is selected."""
        ...
        if not group_ids:
            return
        # Draw a combined bounding box + resize handles for each selected group
        inv_scale = 1.0 / self.current_scale()
        handle_size = 8.0 * inv_scale
        for gid in group_ids:
            ...
            if union.isNull():
                continue
            margin = 4.0 * inv_scale
            box = union.adjusted(-margin, -margin, margin, margin)
            # Dashed selection rectangle
            pen = QPen(QColor(0, 120, 215, 120), 1.0 * inv_scale)
            ...
            painter.setPen(QPen(QColor(0, 120, 215), 1.0 * inv_scale))
            painter.setBrush(QBrush(QColor(255, 255, 255)))
            hs = handle_size
            for hp in self._group_handle_positions(box):
                painter.drawRect(QRectF(hp.x() - hs / 2, hp.y() - hs / 2, hs, hs))

    # -- Group resize helpers --

    @staticmethod
    def _group_handle_positions(box: QRectF) -> list[QPointF]:
        """8 handle positions (TL, TC, TR, ML, MR, BL, BC, BR) for a rect."""
        x, y, w, h = box.x(), box.y(), box.width(), box.height()
        return [
            QPointF(x, y), QPointF(x + w / 2, y), QPointF(x + w, y),
            QPointF(x, y + h / 2), QPointF(x + w, y + h / 2),
            QPointF(x, y + h), QPointF(x + w / 2, y + h), QPointF(x + w, y + h),
        ]

    def _group_handle_hit(self, scene_pos: QPointF):
        """Test if scene_pos hits a group resize handle.

        Returns (group_id, handle_index, group_box) or (None, -1, None).
        """
        from diagrammer.commands.group_command import get_top_group
        selected = self._diagram_scene.selectedItems()
        if not selected:
        ...
        for gid in group_ids:
            ...
            if union.isNull():
                continue
            box = union.adjusted(-margin, -margin, margin, margin)
            for i, hp in enumerate(self._group_handle_positions(box)):
                if abs(scene_pos.x() - hp.x()) < hit_radius and abs(scene_pos.y() - hp.y()) < hit_radius:
                    return gid, i, union  # return content rect, not margin box
        return None, -1, None

    def _snapshot_group_members(self, gid: str) -> list[tuple]:
        """Capture position, size, scene center, scale, and vertices for all group members."""
        from diagrammer.items.connection_item import ConnectionItem
        from diagrammer.items.svg_image_item import SvgImageItem
        from diagrammer.items.shape_item import ShapeItem, LineItem
        snaps = []
        for m in self._diagram_scene.get_group_members(gid):
            pos = QPointF(m.pos())
            center = m.sceneBoundingRect().center()
            orig_scale = m.scale()
            w = getattr(m, '_width', None) or getattr(m, 'image_width', None)
            h = getattr(m, '_height', None) or getattr(m, 'image_height', None)
            verts = [QPointF(v) for v in m.vertices] if isinstance(m, ConnectionItem) else None
            # For LineItem, capture start/end points
            if isinstance(m, LineItem):
                w = QPointF(m._start)
                h = QPointF(m._end)
            snaps.append((m, pos, center, orig_scale, w, h, verts))
        return snaps

    def _apply_group_resize(self, anchor: QPointF, sx: float, sy: float) -> None:
        """Scale all group members' positions and sizes relative to anchor.

        Uses each item's sceneBoundingRect center (not pos()) to compute
        the move delta, so rotated/transformed items stay visually coherent.
        Items that support explicit resize (SvgImageItem, ShapeItem) get
        width/height scaled; others (ComponentItem, etc.) get setScale().
        """
        from diagrammer.items.connection_item import ConnectionItem
        from diagrammer.items.svg_image_item import SvgImageItem
        from diagrammer.items.shape_item import ShapeItem, LineItem

        # Uniform scale for items that use setScale()
        uniform = max(sx, sy) if sx == sy else (sx + sy) / 2.0

        for m, orig_pos, orig_center, orig_scale, orig_w, orig_h, orig_verts in self._group_resize_member_starts:
            # Where this item's visual center should end up
            desired_cx = anchor.x() + (orig_center.x() - anchor.x()) * sx
            desired_cy = anchor.y() + (orig_center.y() - anchor.y()) * sy

            if isinstance(m, ConnectionItem) and orig_verts is not None:
                new_verts = []
                for v in orig_verts:
                    new_verts.append(QPointF(
            ...
            # 1) Restore original state so we can measure correctly
            if hasattr(m, '_skip_snap'):
                m._skip_snap = True
            elif hasattr(m, '_skip_snap_once'):
                m._skip_snap_once = True
            m.setPos(orig_pos)
            m.setScale(orig_scale)
            if isinstance(m, SvgImageItem) and isinstance(orig_w, (int, float)):
                m.prepareGeometryChange()
                m._width = orig_w
                m._height = orig_h
            elif isinstance(m, ShapeItem) and isinstance(orig_w, (int, float)):
                m.prepareGeometryChange()
                m._width = orig_w
                m._height = orig_h
            elif isinstance(m, LineItem) and isinstance(orig_w, QPointF):
                m.prepareGeometryChange()
                m._start = QPointF(orig_w)
                m._end = QPointF(orig_h)

            # 2) Apply size/scale changes
            if isinstance(m, SvgImageItem) and isinstance(orig_w, (int, float)):
                m.prepareGeometryChange()
                m._width = max(10.0, orig_w * sx)
                m._height = max(10.0, orig_h * sy)
            elif isinstance(m, LineItem) and isinstance(orig_w, QPointF):
                m.prepareGeometryChange()
                m._start = QPointF(orig_w.x() * sx, orig_w.y() * sy)
                m._end = QPointF(orig_h.x() * sx, orig_h.y() * sy)
            elif isinstance(m, ShapeItem) and isinstance(orig_w, (int, float)):
                m.prepareGeometryChange()
                m._width = max(10.0, orig_w * sx)
                m._height = max(10.0, orig_h * sy)
            else:
                m.setScale(orig_scale * uniform)

            # 3) Ask Qt where the center is NOW (after size/scale change
            #    but before position change), then nudge pos to place it
            #    at the desired center.
            current_center = m.sceneBoundingRect().center()
            nudge_x = desired_cx - current_center.x()
            nudge_y = desired_cy - current_center.y()
            m.setPos(orig_pos.x() + nudge_x, orig_pos.y() + nudge_y)
    ...
    def _finalize_group_resize(self) -> None:
        """Push an undo command that captures the before/after state of a group resize."""
        from PySide6.QtGui import QUndoCommand
        from diagrammer.items.connection_item import ConnectionItem
        from diagrammer.items.svg_image_item import SvgImageItem

        class _GroupResizeCommand(QUndoCommand):
            def __init__(self, members_before, scene):
                super().__init__()
                self.setText("Resize group")
                self._scene = scene
                # Capture before and after state for each member
                self._states = []
                for m, orig_pos, orig_center, orig_scale, orig_w, orig_h, orig_verts in members_before:
                    after_pos = QPointF(m.pos())
                    after_scale = m.scale()
                    after_w = getattr(m, '_width', None) or getattr(m, 'image_width', None)
                    after_h = getattr(m, '_height', None) or getattr(m, 'image_height', None)
                    after_verts = [QPointF(v) for v in m.vertices] if isinstance(m, ConnectionItem) else None
                    self._states.append((
                        m, orig_pos, orig_scale, orig_w, orig_h, orig_verts,
                        after_pos, after_scale, after_w, after_h, after_verts,
                    ))

            def redo(self):
                for (m, _, _, _, _, _,
                     a_pos, a_scale, a_w, a_h, a_verts) in self._states:
                    if hasattr(m, '_skip_snap'):
                        m._skip_snap = True
                    m.setPos(a_pos)
                    m.setScale(a_scale)
                    if hasattr(m, '_skip_snap'):
                        m._skip_snap = False
                    if isinstance(m, SvgImageItem) and a_w is not None:
                        m.prepareGeometryChange()
                        m._width = a_w
                        m._height = a_h
                        m.update()
            ...
            def undo(self):
                for (m, o_pos, o_scale, o_w, o_h, o_verts,
                     _, _, _, _, _) in self._states:
                    if hasattr(m, '_skip_snap'):
                        m._skip_snap = True
                    m.setPos(o_pos)
                    m.setScale(o_scale)
                    if hasattr(m, '_skip_snap'):
                        m._skip_snap = False
                    if isinstance(m, SvgImageItem) and isinstance(o_w, (int, float)):
                        m.prepareGeometryChange()
                        m._width = o_w
                        m._height = o_h
                        m.update()
        ...
        changed = any(
            m.pos() != orig_pos or m.scale() != orig_scale
            for m, orig_pos, _, orig_scale, _, _, _ in self._group_resize_member_starts
        )
        if changed:
            cmd = _GroupResizeCommand(self._group_resize_member_starts, self._diagram_scene)
    ...
    def mousePressEvent(self, event) -> None:
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            ...
            # Group resize handle check — before any item-specific handling
            gid, hi, gbox = self._group_handle_hit(scene_pos)
            if gid is not None and hi >= 0:
                self._group_resizing = True
                self._group_resize_handle = hi
                self._group_resize_gid = gid
                self._group_resize_start_rect = QRectF(gbox)
                self._group_resize_start_mouse = QPointF(scene_pos)
                self._group_resize_member_starts = self._snapshot_group_members(gid)
                self.setDragMode(QGraphicsView.DragMode.NoDrag)
                event.accept()
                return
            ...
            if event.modifiers() & Qt.KeyboardModifier.ShiftModifier:
                ...
                elif isinstance(item, (ComponentItem, ConnectionItem, ShapeItem, LineItem, JunctionItem, AnnotationItem, SvgImageItem)):
                    if item.isSelected():
            ...
            # (Annotation/shape/line drags are handled by the unified
            # "movable item" branch below — see the isinstance() check
            # that also covers ComponentItem and JunctionItem.)
                        ...
                        for si in self._diagram_scene.items():
                            if isinstance(si, ConnectionItem) and id(si) not in seen_conn_ids:
                                ...
                                if src_in and tgt_in:
                                    ...
                                    seen_conn_ids.add(id(si))
                        self._trim_external_conn_endpoint_wps(comp_ids, seen_conn_ids)
                        self.setDragMode(QGraphicsView.DragMode.NoDrag)
                        event.accept()
                        return

            # -- Click on a movable item → set up group-aware drag --
            # Covers ComponentItem, JunctionItem, AnnotationItem, and
            # ShapeItem/LineItem so that grouped shapes and annotations
            # move in lockstep with the rest of the group instead of
            # drifting off on their own via Qt's default item drag.
            else:
                from diagrammer.items.shape_item import (
                    LineItem as _DragLineItem,
                    ShapeItem as _DragShapeItem,
                )
                _movable_types = (
                    ComponentItem, JunctionItem, AnnotationItem,
                    _DragShapeItem, _DragLineItem,
                )
                # If the click lands on a shape's resize handle or a
                # line's endpoint, defer to the item's own mousePress
                # so resize / endpoint-edit gestures still work.
                _handle_hit = False
                if isinstance(item, _DragShapeItem) and item.isSelected():
                    _local = item.mapFromScene(scene_pos)
                    if item._handle_at(_local) is not None:
                        _handle_hit = True
                elif isinstance(item, _DragLineItem) and item.isSelected():
                    _local = item.mapFromScene(scene_pos)
                    if item._endpoint_at(_local) is not None:
                        _handle_hit = True

                if isinstance(item, _movable_types) and not _handle_hit:
    ...
    def mouseMoveEvent(self, event) -> None:
        # Group resize drag
        if self._group_resizing and self._group_resize_start_mouse is not None:
            scene_pos = self.mapToScene(event.position().toPoint())
            delta = QPointF(scene_pos.x() - self._group_resize_start_mouse.x(),
                            scene_pos.y() - self._group_resize_start_mouse.y())
            r = self._group_resize_start_rect
            hi = self._group_resize_handle

            # Compute new box dimensions based on handle
            new_left, new_top = r.left(), r.top()
            new_right, new_bottom = r.right(), r.bottom()
            # Left handles (0, 3, 5)
            if hi in (0, 3, 5):
                new_left = min(r.left() + delta.x(), r.right() - 10)
            # Right handles (2, 4, 7)
            if hi in (2, 4, 7):
                new_right = max(r.right() + delta.x(), r.left() + 10)
            # Top handles (0, 1, 2)
            if hi in (0, 1, 2):
                new_top = min(r.top() + delta.y(), r.bottom() - 10)
            # Bottom handles (5, 6, 7)
            if hi in (5, 6, 7):
                new_bottom = max(r.bottom() + delta.y(), r.top() + 10)

            # Corner handles: proportional by default, Shift = free-form
            is_corner = hi in (0, 2, 5, 7)
            free_form = bool(event.modifiers() & Qt.KeyboardModifier.ShiftModifier)
            if is_corner and not free_form:
                aspect = r.width() / r.height() if r.height() > 0 else 1.0
                nw = new_right - new_left
                nh = new_bottom - new_top
                sw = nw / r.width() if r.width() > 0 else 1.0
                sh = nh / r.height() if r.height() > 0 else 1.0
                if abs(sw - 1.0) >= abs(sh - 1.0):
                    nh = nw / aspect
                else:
                    nw = nh * aspect
                if hi in (0, 5):
                    new_left = new_right - nw
                else:
                    new_right = new_left + nw
                if hi in (0, 2):
                    new_top = new_bottom - nh
                else:
                    new_bottom = new_top + nh

            sx = (new_right - new_left) / r.width() if r.width() > 0 else 1.0
            sy = (new_bottom - new_top) / r.height() if r.height() > 0 else 1.0
            anchor = QPointF(
                new_right if hi in (0, 3, 5) else new_left if hi in (2, 4, 7) else r.left(),
                new_bottom if hi in (0, 1, 2) else new_top if hi in (5, 6, 7) else r.top(),
            )
            # Anchor in original coords for scaling member positions
            orig_anchor = QPointF(
                r.right() if hi in (0, 3, 5) else r.left(),
                r.bottom() if hi in (0, 1, 2) else r.top(),
            )
            self._apply_group_resize(orig_anchor, sx, sy)
            self.viewport().update()
            event.accept()
            return
    ...
    def mouseReleaseEvent(self, event) -> None:
        ...
        if event.button() == Qt.MouseButton.LeftButton:
            # Finish group resize — push undo command
            if self._group_resizing:
                try:
                    self._finalize_group_resize()
                finally:
                    self._group_resizing = False
                    self._group_resize_handle = -1
                    self._group_resize_gid = None
                    self._group_resize_start_rect = None
                    self._group_resize_start_mouse = None
                    self._group_resize_member_starts = []
                    self.setDragMode(QGraphicsView.DragMode.RubberBandDrag)
                    self.viewport().update()
                event.accept()
                return
                ...
                try:
                    ...
                    for conn in self._drag_internal_conns:
                        ...
                        if orig_wps != new_wps:
                            cmd = EditWaypointsCommand(conn, orig_wps, new_wps)
                            self._diagram_scene.undo_stack.push(cmd)
                    # Record waypoint changes for external connections whose
                    # stale endpoint waypoints were trimmed at drag start
                    if self._drag_ext_conn_orig_wps:
                        for inst_id, orig_wps in self._drag_ext_conn_orig_wps.items():
                            for item in self._diagram_scene.items():
                                if isinstance(item, ConnectionItem) and item.instance_id == inst_id:
                                    new_wps = [QPointF(w) for w in item.vertices]
                                    if orig_wps != new_wps:
                                        cmd = EditWaypointsCommand(item, orig_wps, new_wps)
                                        self._diagram_scene.undo_stack.push(cmd)
                                    break
                finally:
    ...
    def keyPressEvent(self, event) -> None:
        ...
        # Return/Enter: enter editing mode on a selected (non-editing) annotation
        if event.key() in (Qt.Key.Key_Return, Qt.Key.Key_Enter):
            sel = self._diagram_scene.selectedItems()
            annots = [i for i in sel if isinstance(i, AnnotationItem) and not i.is_editing]
            if len(annots) == 1 and len(sel) == 1:
                annots[0].start_editing()
                event.accept()
                return
    ...
    def _trim_external_conn_endpoint_wps(self, comp_ids: set[int],
                                            seen_conn_ids: set[int]) -> None:
        """Trim stale waypoints near moving endpoints on external connections.

        When a segment drag stores the expanded route as waypoints, port
        positions can end up stored as explicit waypoints.  If the
        component is later dragged, those stale waypoints create trailing
        wire stubs.  This method strips them at drag start and records
        the originals for undo.
        """
        from diagrammer.utils.geometry import point_distance
        self._drag_ext_conn_orig_wps = {}
        for si in self._diagram_scene.items():
            if not isinstance(si, ConnectionItem) or id(si) in seen_conn_ids:
                continue
            ...
            src_in = src_comp is not None and id(src_comp) in comp_ids
            tgt_in = tgt_comp is not None and id(tgt_comp) in comp_ids
            if not (src_in or tgt_in) or (src_in and tgt_in):
                continue  # skip unrelated or internal connections
            if not si._waypoints:
                continue
            orig = [QPointF(w) for w in si._waypoints]
            changed = False
            if tgt_in:
    ...
    def _setup_unified_drag(self, scene_pos: QPointF) -> None:
        ...
        seen_ids = set(id(c) for c in self._drag_internal_conns)
        seen_ids.update(id(self._selected_waypoint_conns[cid])
                        for cid in unified_conn_ids
                        if cid in self._selected_waypoint_conns)
        self._trim_external_conn_endpoint_wps(comp_ids, seen_ids)
        self.setDragMode(QGraphicsView.DragMode.NoDrag)
    ...
    def _duplicate_selection(self, clicked_item, scene_pos) -> list:
        ...
        if gid:
            ...
            from diagrammer.items.svg_image_item import SvgImageItem as _DupSvg
            selected = [
                i for i in self._diagram_scene.selectedItems()
                if isinstance(
                    i,
                    (ComponentItem, JunctionItem, AnnotationItem,
                     ShapeItem, LineItem, _DupSvg),
                )
            ]
    ...
    def _duplicate_item(self, item):
        ...
        """
        from diagrammer.items.svg_image_item import SvgImageItem as _SvgImageItem
        clone = None
        if isinstance(item, ComponentItem):
            ...
            if clone:
                ...
                if not item.style_overrides.is_empty():
                    import copy
                    clone.set_style_overrides(copy.deepcopy(item.style_overrides))
        ...
        if clone is not None and not isinstance(item, ComponentItem):
            from diagrammer.commands.shape_command import AddShapeCommand
            cmd = AddShapeCommand(self._diagram_scene, clone, item.pos())
            self._diagram_scene.undo_stack.push(cmd)
    ...
    def contextMenuEvent(self, event) -> None:
        ...
        # Move to Layer
        if selected:
            layer_mgr = self._diagram_scene.layer_manager
            if len(layer_mgr.layers) > 1:
                if menu.actions():
                    menu.addSeparator()
                move_menu = menu.addMenu("Move to Layer")
                current_layers = {getattr(i, '_layer_index', 0) for i in selected}
                for idx, layer in enumerate(layer_mgr.layers):
                    label = layer.name
                    if idx in current_layers:
                        label = f"\u2022 {label}"  # bullet marks current
                    act = move_menu.addAction(label)
                    # Disable when every selected item is already on this layer
                    if len(current_layers) == 1 and idx in current_layers:
                        act.setEnabled(False)
                    act.triggered.connect(
                        lambda _checked=False, i=idx: self._move_selection_to_layer(i)
                    )
    ...
    def _move_selection_to_layer(self, layer_index: int) -> None:
        ...
        from diagrammer.commands.layer_command import MoveToLayerCommand
        items = list(self._diagram_scene.selectedItems())
        if not items:
            return
        cmd = MoveToLayerCommand(self._diagram_scene, items, layer_index)
        self._diagram_scene.undo_stack.push(cmd)
