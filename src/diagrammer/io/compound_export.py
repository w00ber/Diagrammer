...
def export_compound_component(
    ...
    svg_images = [i for i in selected_items if isinstance(i, SvgImageItem)]

    if not components and not annotations and not shapes and not svg_images:
        return False
    ...
    all_items = components + connections + junctions + annotations + shapes + svg_images
    ...
    for item in render_items:
        ...
        elif isinstance(item, SvgImageItem):
            _render_svg_image(artwork, item, ox, oy)
...
def _inline_math_annotation(parent: ET.Element, annot, x: float, y: float,
    ...
    try:
        ...
        logger.debug("LaTeX rendering failed for annotation in compound export", exc_info=True)
        return False
    if not svg_bytes:
        return False
...
def _render_shape(parent: ET.Element, shape, ox: float, oy: float) -> None:
    ...
    if isinstance(shape, RectangleItem):
        ...
        if rotation:
            cx = x + shape.shape_width / 2
            cy = y + shape.shape_height / 2
            rect.set("transform", f"rotate({rotation:.1f},{cx:.1f},{cy:.1f})")
        ...
        if rotation:
            cx = x + rx
            cy = y + ry
            ellipse.set("transform", f"rotate({rotation:.1f},{cx:.1f},{cy:.1f})")
...
def _render_svg_image(parent: ET.Element, item, ox: float, oy: float) -> None:
    """Render an SvgImageItem by inlining its SVG content at its scene position/size."""
    import xml.etree.ElementTree as ET2
    ...
    try:
        root = ET2.fromstring(item.svg_data)
    except ET2.ParseError:
        return

    # Strip SVG namespace prefixes so elements serialize unprefixed
    _SVG_NS = "{http://www.w3.org/2000/svg}"
    for el in root.iter():
        if isinstance(el.tag, str) and el.tag.startswith(_SVG_NS):
            el.tag = el.tag[len(_SVG_NS):]

    # Get the SVG's native viewBox dimensions for scaling
    vb = root.get("viewBox", "")
    vx = vy = 0.0
    vw = vh = 0.0
    if vb:
    ...
    # Build a transform that positions and scales the SVG content
    # to match the item's current size on the canvas
    g = ET2.SubElement(parent, "g")
    transform_parts = [f"translate({x:.1f},{y:.1f})"]
    if vw > 0 and vh > 0:
        sx = item.image_width / vw
        sy = item.image_height / vh
        transform_parts.append(f"scale({sx:.4f},{sy:.4f})")
        if vx != 0 or vy != 0:
            transform_parts.append(f"translate({-vx:.1f},{-vy:.1f})")
    g.set("transform", " ".join(transform_parts))

    # Copy all children (defs, groups, paths, etc.) into our group
    for child in list(root):
        g.append(child)
