...
def save_compound_manifest(
    ...
    if not (components or annotations or shapes or svg_images):
        return False
    ...
    all_items = components + junctions + connections + annotations + shapes + svg_images
    union = None
    ...
    svg_image_blobs = [_serialize_svg_image(si) for si in svg_images]

    for blob in comp_blobs + junc_blobs + annot_blobs + shape_blobs + svg_image_blobs:
        x, y = blob["pos"]
        blob["pos"] = [x - ox, y - oy]
...
def load_compound_manifest(path: Path) -> dict | None:
    ...
    try:
        ...
        logger.warning("Failed to load compound manifest %s", path, exc_info=True)
        return None
