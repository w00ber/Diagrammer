...
class DiagramSerializer:
    ...
    def save(scene: QGraphicsScene, path: str | Path) -> None:
        ...
        # Serialize SVG images
        for item in scene.items():
            if isinstance(item, SvgImageItem):
                data["svg_images"].append(_serialize_svg_image(item))
    ...
    @staticmethod
    def load(scene: QGraphicsScene, path: str | Path, library=None) -> list[str]:
        """Load a .dgm JSON file and populate the scene.

        Returns a list of ``def_key`` strings for components that were
        reconstructed from embedded SVG data because they were not found
        ...
        # Load components
        embedded_fallback_keys: list[str] = []
        for cd in data.get("components", []):
            comp_def = library.get(cd["def_key"]) if library else None
            if comp_def is None and "embedded_svg" in cd:
                try:
                    from diagrammer.models.component_def import ComponentDef
                    svg_bytes = base64.b64decode(cd["embedded_svg"])
                    meta = cd.get("embedded_meta", {})
                    comp_def = ComponentDef.from_embedded(meta, svg_bytes)
                    embedded_fallback_keys.append(cd["def_key"])
                except (ValueError, KeyError, OSError) as exc:
                    logger.debug("Failed to reconstruct embedded component %s: %s",
                                 cd.get("def_key", "?"), exc)
                    comp_def = None
            if comp_def is None:
                continue
        ...
        for sid in data.get("svg_images", []):
            ...
            item = SvgImageItem(
                svg_data,
                width=sid.get("width"),
                height=sid.get("height"),
                instance_id=sid.get("id"),
            )
            item.setPos(QPointF(sid["pos"][0], sid["pos"][1]))
            item._layer_index = sid.get("layer", 0)
            scene.addItem(item)
            _restore_group(item, sid)
        ...
        # Recompute Z values from layer index (Illustrator semantics).
        # Saved per-item z values are now derived data and intentionally ignored.
        from diagrammer.canvas.scene import DiagramScene
        if isinstance(scene, DiagramScene):
            scene.reflow_z()
            scene.update_connections()

        return embedded_fallback_keys
...
def _serialize_component(item) -> dict:
    ...
    # Embed SVG data for portability (best-effort)
    comp_def = item.component_def
    try:
        svg_data = comp_def.svg_bytes or comp_def.svg_path.read_bytes()
        d["embedded_svg"] = base64.b64encode(svg_data).decode("ascii")
        d["embedded_meta"] = {"name": comp_def.name, "category": comp_def.category}
    except (OSError, AttributeError) as exc:
        logger.debug("Failed to embed SVG data for component: %s", exc)
    return d
...
def _serialize_svg_image(item) -> dict:
    ...
    return {
        ...
        "svg_data": base64.b64encode(item.svg_data).decode("ascii"),
        "layer": getattr(item, '_layer_index', 0),
        "z": item.zValue(),
        "group": getattr(item, '_group_ids', []) or [],
    }
