...
class ComponentItem(QGraphicsItem):
    ...
    def _prepare_svg_bytes(svg_path, lead_shortening: dict[str, float] | None = None,
        ...
        # Inline CSS url() references (e.g. gradient fills) as direct SVG
        # attributes so that downstream SVG manipulations (style overrides,
        # compound export) can see them without parsing the <style> block.
        ComponentItem._inline_css_url_refs(root)
    ...
    @staticmethod
    def _inline_css_url_refs(root) -> None:
        """Inline CSS class rules that contain ``url()`` refs as SVG attributes.

        Illustrator-exported SVGs typically reference gradients via a CSS
        class::

            .st0 { fill: url(#linear-gradient); }
            <path class="st0" .../>

        Qt's ``QSvgRenderer`` resolves these correctly for screen / QImage
        targets, but ``QPrinter``'s PDF backend silently drops CSS-resolved
        gradient fills.  By converting to an inline attribute::

            <path fill="url(#linear-gradient)" .../>

        the gradient travels as a first-class SVG attribute that Qt's PDF
        paint engine can translate to a native PDF gradient object.

        Only CSS properties whose values contain ``url(`` are inlined; plain
        colour / stroke rules are left in the stylesheet so that
        ``_apply_style_overrides`` (which already knows how to merge CSS +
        ...
        # 1) Parse <style> blocks and extract class → {prop: value} where
        #    value contains a url() reference.
        url_rules: dict[str, dict[str, str]] = {}  # className → {prop: value}
        for elem in root.iter():
                ...
                for m in re.finditer(r'([^{}]+)\{([^}]+)\}', elem.text):
                    ...
                    for cls_match in re.finditer(r'\.([\w-]+)', selectors_str):
                        ...
                        for prop_match in re.finditer(
                                r'([\w-]+)\s*:\s*([^;]*url\s*\([^)]+\)[^;]*)', props_str):
                            url_rules.setdefault(cls_name, {})[
                                prop_match.group(1).strip()] = prop_match.group(2).strip()

        if not url_rules:
            return

        # 2) Walk every element; if it carries a class that maps to a url()
        #    rule, set the property as a direct attribute.
        for elem in root.iter():
            cls_attr = elem.get("class", "")
            if not cls_attr:
                continue
            for cls in cls_attr.split():
                props = url_rules.get(cls)
                if not props:
                    continue
                for prop, value in props.items():
                    # Only set if the element doesn't already carry an
                    # explicit attribute (explicit wins).
