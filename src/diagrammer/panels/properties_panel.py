...
class PropertiesPanel(QDockWidget):
    ...
    def _set_wire_defaults(self, item) -> None:
        ...
        app_settings.save()
        if self._scene:
            self._scene.default_routing_mode = item.routing_mode
    ...
    def _rebuild_isolation_controls(self, item, element_paths) -> None:
        ...
        def _effective_val(path, prop, css_prop):
            ...
            # Skip transient flash highlight values — fall through to CSS
            if val == _FLASH_COLOR:
                val = None
            if val is None:
        ...
        def _with_default(val, default):
            return val if val is not None else default
        stroke_vals = {_with_default(_effective_val(p, 'stroke_color', 'stroke'), 'none') for p in paths}
        fill_vals = {_with_default(_effective_val(p, 'fill_color', 'fill'), '#000000') for p in paths}
        width_vals = set()
        for p in paths:
        ...
        fill_val = next(iter(fill_vals)) if not fill_mixed else '#000000'
        ...
        stroke_is_none = stroke_val == "none"
        current_stroke = QColor(stroke_val) if not stroke_is_none else QColor(0, 0, 0)
        if stroke_mixed:
        ...
        stroke_none_btn.clicked.connect(
            lambda checked=False, it=item, p=paths, btn=stroke_btn: (
                self._set_isolation_style(it, p, 'stroke_color', 'none'),
                btn.setStyleSheet(self._NONE_SWATCH_STYLE),
            )
        )
        ...
        fill_is_none = (not fill_val) or fill_val == "none"
        current_fill = QColor(fill_val) if not fill_is_none else QColor(255, 255, 255, 0)
        if fill_mixed:
        ...
        fill_none_btn.clicked.connect(
            lambda checked=False, it=item, p=paths, btn=fill_btn: (
                self._set_isolation_style(it, p, 'fill_color', 'none'),
                btn.setStyleSheet(self._NONE_SWATCH_STYLE),
            )
        )
    ...
    def _flash_element(self, item, element_path: str) -> None:
        ...
        Uses a separate flash overlay dict so the real style overrides
        are never mutated — preventing stale flash colors from appearing
        in rebuilt controls.
        """
        ...
        # Save the real override so we can restore after each flash-ON
        ...
        def _do_flash():
            if count[0] >= 6:
                # Final restore — put back the real override
                if saved_ovr:
            ...
            if count[0] % 2 == 0:
                # Flash ON — use a temporary override for visual only
                flash_ovr = copy.deepcopy(saved_ovr) if saved_ovr else SvgElementStyleOverride()
                flash_ovr.stroke_color = flash_color
                flash_ovr.fill_color = flash_color
                item._style_overrides.overrides[element_path] = flash_ovr
            else:
                # Flash OFF — restore the real override (re-read in case
                # the user changed it since the flash started)
                real_ovr = item.style_overrides.get(element_path)
                # Only restore saved if the current value is our flash override
                current_stroke = getattr(real_ovr, 'stroke_color', None) if real_ovr else None
                if current_stroke == flash_color:
                    if saved_ovr:
                        item._style_overrides.overrides[element_path] = copy.deepcopy(saved_ovr)
                    else:
                        item._style_overrides.overrides.pop(element_path, None)
    ...
    _NONE_SWATCH_STYLE = (
        ...
        "stop:0 white,stop:0.42 white,stop:0.43 red,"
        "stop:0.57 red,stop:0.58 white,stop:1 white);"
        "border: 1px solid #888;"
    )

    def _make_color_button(self, color: QColor, callback, alpha: bool = False,
                           is_none: bool = False) -> QPushButton:
        btn = QPushButton()
